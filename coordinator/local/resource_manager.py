...
class Job:
    def __init__(self, name, priority=0, device=0, required_gpu=0):
        self.name_ = name
...
class ResourceManager:
    ...
    def __distribute_one_gpu(self,
        ...
        简化版单卡资源分配策略：
        1. 目前仅有两优先级, 高优0, 低优1
        2. 高优作业按照 required 分配, 上层调度需要保证不超发
        ...
        if high_required > gpu_memory_total:
            logging.warn(
        ...
        for j in high_jobs:
            distribution[j.name_] = {
                ...
                'maxDeviceMemMb': j.required_gpu_
            }
