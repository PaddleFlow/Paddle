...
void CalculateMd5(const unsigned char* buffer, size_t buffer_size,
                  unsigned char* md) {
...
FileInfo::FileInfo(const std::string& file_path) : path_(file_path) {
...
bool FileInfo::IsFileChanged() {
  ...
  if (stat(path_.c_str(), &file_stat) == 0) {
    ...
    if (IsTimeElapsed(last_checked_time_, kFileContentCheckTimeoutInSec)) {
      last_checked_time_ = {time(0), 0};
...
void FileListener::StartMonitorThread() {
  stop_running_ = false;
  file_monitor_thread_ = new std::thread(&FileListener::FileMonitor, this);
...
void FileListener::UnregisterFileListener(const std::string& file_path,
                                          const std::string& handler_name) {
  VLOG(2) << "Unregister a file listener named " << handler_name << " on file "
          << file_path;
  ...
  for (const auto& listener : listeners_) {
    for (const auto& handler : listener.second.file_handlers_) {
      VLOG(vlog_level) << "Registered file listener named "
                       << handler.func_name_ << " on file path "
                       << listener.first;
    }
  }
}
